...
#!unzip -qq /content/gdrive/MyDrive/flickr30k_images.zip
...
#Flickr30k Dataset
train_images_list = os.listdir('/content/flickr30k_images/')
...
for i in train_images_list:
    real_images[j] = np.array(plt.imread('/content/flickr30k_images/' + i))
    train[j] = np.array(plt.imread('/content/flickr30k_images/' + i))
    j += 1    
...
#PCA Dataset
train_images_listPCA = os.listdir('/content/PCA_dataset/')
train_images_listPCA = train_images_listPCA[:sample_size]

trainPCA = np.array([None] * sample_size)
real_imagesPCA = np.array([None] * sample_size)

j = 0
for i in train_images_listPCA:
    real_imagesPCA[j] = np.array(plt.imread('/content/PCA_dataset/' + i))
    trainPCA[j] = np.array(plt.imread('/content/PCA_dataset/' + i))
    j += 1    


j = 0
for i in trainPCA:
    trainPCA[j] = cv2.resize(i, size)
    trainPCA[j] = trainPCA[j].reshape(1, size[0], size[1], num_channels)
    j += 1

trainPCA = np.vstack(trainPCA[:])



#SVD Dataset
train_images_listSVD = os.listdir('/content/SVD_dataset/')
train_images_listSVD = train_images_listSVD[:sample_size]

trainSVD = np.array([None] * sample_size)
real_imagesSVD = np.array([None] * sample_size)

j = 0
for i in train_images_listSVD:
    real_imagesSVD[j] = np.array(plt.imread('/content/SVD_dataset/' + i))
    trainSVD[j] = np.array(plt.imread('/content/SVD_dataset/' + i))
    j += 1    


j = 0
for i in trainSVD:
    trainSVD[j] = cv2.resize(i, size)
    trainSVD[j] = trainSVD[j].reshape(1, size[0], size[1], num_channels)
    j += 1

trainSVD = np.vstack(trainSVD[:])


#KM Dataset
train_images_listKM = os.listdir('/content/KM_dataset/')
train_images_listKM = train_images_listKM[:sample_size]

trainKM = np.array([None] * sample_size)
real_imagesKM = np.array([None] * sample_size)

j = 0
for i in train_images_listKM:
    real_imagesKM[j] = np.array(plt.imread('/content/KM_dataset/' + i))
    trainKM[j] = np.array(plt.imread('/content/KM_dataset/' + i))
    j += 1    


j = 0
for i in trainKM:
    trainKM[j] = cv2.resize(i, size)
    trainKM[j] = trainKM[j].reshape(1, size[0], size[1], num_channels)
    j += 1

trainKM = np.vstack(trainKM[:])


#Original Captions
train_captions = pd.read_csv('/content/gdrive/MyDrive/results.csv', delimiter='|')
...
with tf.Session() as sess:
    ...
    for image_num in range(num_tests):
        ...
        print('Time for Predicted Caption:->',endOriginal - startOriginal)
        ...
        #PCA dataset Prediction
        startPCA = timer()
        caption = sess.run(final_prediction, feed_dict = {x_inp:trainPCA[image_num:image_num + 1]})
        endPCA = timer()
        caption = np.argmax(caption[:-1], 1)
        capt = ''
        for i in caption:
            capt += rev_dict[i] + ' '
        print('Predicted Caption PCA:->', capt)
        print('Time for Predicted Caption PCA:->',endPCA - startPCA)
        print('Orignal Caption:->', orignalcaption)
        plt.imshow(real_imagesPCA[image_num])
        plt.title('Image using PCA')
        plt.show()


        #SVD dataset Prediction
        startSVD = timer()
        caption = sess.run(final_prediction, feed_dict = {x_inp:trainSVD[image_num:image_num + 1]})
        endSVD = timer()
        caption = np.argmax(caption[:-1], 1)
        capt = ''
        for i in caption:
            capt += rev_dict[i] + ' '
        print('Predicted Caption SVD:->', capt)
        print('Time for Predicted Caption SVD:->',endSVD - startSVD)
        print('Orignal Caption:->', orignalcaption)
        plt.imshow(real_imagesSVD[image_num])
        plt.title('Image using SVD')
        plt.show()


        #KM dataset Prediction
        startKM = timer()
        caption = sess.run(final_prediction, feed_dict = {x_inp:trainKM[image_num:image_num + 1]})
        endKM = timer()
        caption = np.argmax(caption[:-1], 1)
        capt = ''
        for i in caption:
            capt += rev_dict[i] + ' '
        print('Predicted Caption KM:->', capt)
        print('Time for Predicted Caption KM:->',endKM - startKM)
        print('Orignal Caption:->', orignalcaption)
        plt.imshow(real_imagesKM[image_num])
        plt.title('Image using KM')
        plt.show()